...
class ImgTrackColorJitter2(torchvision.transforms.ColorJitter):
    def __init__(self, brightness=0, contrast=0, saturation=0, hue=0):
        super().__init__(brightness=brightness, contrast=contrast, saturation=saturation, hue=hue)

    def forward(self, inputs):
        img, tracks, masks = inputs
        img = super().forward(img)
        return img, tracks, masks
...
class ImgViewDiffTranslationAug2(nn.Module):
    """
    Utilize the random crop from robomimic. Take the same translation for a batch of images.
    """
    ...
    def forward(self, inputs):
        ...
        batch_size, temporal_len, img_c, img_h, img_w = img.shape
        img = img.reshape(batch_size, temporal_len * img_c, img_h, img_w)
        mask_c = masks.shape[2]
        masks = masks.reshape(batch_size, 4 * mask_c, img_h, img_w)
        out = torch.cat([img, masks], dim=1)
        out = F.pad(out, pad=(self.pad_translation,) * 4, mode="replicate")
        out, crop_inds = self.crop_randomizer._forward_in(out, return_crop_inds=True)  # crop_inds: (b, num_crops, 2), where we already set num_crops=1
        img = out[:, :temporal_len * img_c]
        masks = out[:, temporal_len * img_c:]
        img = img.reshape(batch_size, temporal_len, img_c, img_h, img_w)
        masks = masks.reshape(batch_size, 4, mask_c, img_h, img_w)
