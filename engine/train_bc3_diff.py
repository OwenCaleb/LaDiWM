...
from ladiwm.dataloader import BCDataset3, BCDataset3Real, get_dataloader
...
def run_one_epoch(fabric,
    ...
    model.train()
    i = 0
    for d_id, (obs, track_obs, track, task_emb, action, extra_states) in tqdm(enumerate(dataloader)):
        if mix_precision:
            obs, track_obs, track, action = obs.bfloat16(), track_obs.bfloat16(), track.bfloat16(), action.bfloat16()
...
def evaluate(model, dataloader, mix_precision=False, tag="val"):
    ...
    for obs, track_obs, track, task_emb, action, extra_states in tqdm(dataloader):
        obs, track_obs, track, action = obs.cuda(), track_obs.cuda(), track.cuda(), action.cuda()
