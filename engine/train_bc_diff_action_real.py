...
from ladiwm.dataloader import get_dataloader, BCDatasetActionReal
...
def main(cfg: DictConfig):
    ...
    train_dataset = BCDatasetActionReal(dataset_dir=cfg.train_dataset, **cfg.dataset_cfg, aug_prob=cfg.aug_prob)
    train_loader = get_dataloader(train_dataset,
                                      mode="train",
                                      num_workers=cfg.num_workers,
                                      batch_size=cfg.batch_size)

    train_vis_dataset = BCDatasetActionReal(dataset_dir=cfg.train_dataset, vis=True, **cfg.dataset_cfg, aug_prob=cfg.aug_prob)
    train_vis_dataloader = get_dataloader(train_vis_dataset,
                                              mode="train",
                                              num_workers=1,
                                              batch_size=1)

    val_dataset = BCDatasetActionReal(dataset_dir=cfg.val_dataset, num_demos=cfg.val_num_demos, **cfg.dataset_cfg, aug_prob=0.)
    val_loader = get_dataloader(val_dataset, mode="val", num_workers=cfg.num_workers, batch_size=cfg.batch_size)

    val_vis_dataset = BCDatasetActionReal(dataset_dir=cfg.val_dataset, num_demos=cfg.val_num_demos, vis=True, **cfg.dataset_cfg, aug_prob=0.)
...
def run_one_epoch(fabric,
    ...
    model.train()
    i = 0
    for d_id, (obs, track, task_emb, action, extra_states, use_action) in tqdm(enumerate(dataloader)):
        if mix_precision:
            obs, track, task_emb, action, use_action = obs.bfloat16(), \
                track.bfloat16(), task_emb.bfloat16(), action.bfloat16(), use_action.bfloat16()
            extra_states = {k: v.bfloat16() for k, v in extra_states.items()}
        # print(obs.shape)
        loss, ret_dict = model.forward_loss(obs, track, task_emb, extra_states, action, use_action)
        if d_id % 50 == 0:
            print(ret_dict)
        optimizer.zero_grad()
...
def evaluate(model, dataloader, mix_precision=False, tag="val"):
    ...
    i = 0
    for obs, track, task_emb, action, extra_states, use_action in tqdm(dataloader):
        obs, track, task_emb, action, use_action = obs.cuda(), track.cuda(), task_emb, action.cuda(), use_action.cuda()
        extra_states = {k: v.cuda() for k, v in extra_states.items()}
        if mix_precision:
            obs, track, task_emb, action = obs.bfloat16(), track.bfloat16(), task_emb.bfloat16(), action.bfloat16(), use_action.bfloat16()
            extra_states = {k: v.bfloat16() for k, v in extra_states.items()}

        _, ret_dict = model.forward_loss(obs, track, task_emb, extra_states, action, use_action)
